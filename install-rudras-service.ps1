...
if ([string]::IsNullOrWhiteSpace($AdminToken)) {
    ...
    Write-Host ""
    Write-Host "🔑 Generated API Admin Token: $AdminToken" -ForegroundColor Yellow
...
# Set via registry (sc.exe doesn't support env vars directly)
...
if ($status -eq "Running") {
    ...
    Write-Host ""
    Write-Host "⚠️  Service registered but status is '$status'" -ForegroundColor Yellow
