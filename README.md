...
To view the complete manual locally:
...
2. Build and launch the documentation server:
    ...
    ```
3. Read the documentation directly in your browser with a fully-searchable interface.

A packaged copy is also available as a [PDF walkthrough](https://github.com/DeepakKrishna-DK/Rudras-Cognitive_Immunological_Defense_Firewall/blob/main/Rudras_%20The%20Firewall%20in%20Practice.pdf).
...
10. [🚀 Quickstart & Deployment](#-quickstart--deployment)
...
Rudras (named after the ancient storm deity — fierce, adaptive, unstoppable) is a **Cognitive Immunological Defense Firewall**, fully implemented in Rust. It addresses the fundamental flaw of classical rule-based firewalls by introducing a living, self-adapting security system.

Unlike systems that strictly monitor static blacklists, Rudras:

- 👁️ **Observes** every packet flowing through the network interface in real time, using WFP zero-userspace ring-0 enforcement.
- 🔬 **Analyzes** behavioral patterns, threat signatures, and contextual intent (mapped against OWASP A01–A10 and MITRE ATT&CK).
...
- 🌐 **Shares** that intelligence securely with peer nodes across a distributed P2P framework.
- 📊 **Visualizes** everything via a modern, futuristic **Next.js SOC Dashboard**.
...
- **IDS/IPS Taxonomy** — 85+ rules across 70+ threat categories.
...
For the exhaustive breakdown of all 45+ modules, see the [Core Enterprise Capabilities](#️-core-enterprise-capabilities-v30–41) table below and the **[Documentation Hub](./docs)**.
...
Administrators are strongly advised to consult legal and compliance teams before enabling any of these flags. Modifying `config/rudras.toml` to actively intercept third-party packets or auto-terminate host process threads should be strictly checked against your active legal jurisdictions before use.

---

## 🚀 Quickstart & Deployment

### 1. Build and Run the Firewall Engine

Requires **Rust + Cargo, MSVC Build Tools 2022, and the Npcap Driver** (see [Build Requirements](#-build-requirements) below).
...
Or skip the interactive prompt with an explicit mode flag:
